...
class DscPatchEmb2D(nn.Module):
    def __init__(self, in_ch, embed_dim, kernel_size, patch_size, latent_channels=2):
        super().__init__()  
        self.DSConv_x = DSConv(
            in_channels = in_ch,
    ...
    def forward(self, x):
        shift1 = self.DSConv_y(x)
        shift2 = self.DSConv_x(x)
        noshift = self.common_conv(x)
        
        final = self.compress(torch.cat((shift1,shift2,noshift),1))
        embedings = self.patch_embeddings(final)
        return embedings
...
def get_coordinate_map_2D(
    ...
    y_center_ = torch.arange(0, width, dtype=torch.float32).to(device)
    y_center_ = einops.repeat(y_center_, "w -> k w h", k=kernel_size, h=height)

    x_center_ = torch.arange(0, height, dtype=torch.float32).to(device)
    ...
    if morph == 0:
        ...
        """
        y_spread_ = torch.zeros([kernel_size]).to(device)
        x_spread_ = torch.linspace(-center, center, kernel_size).to(device)
        ...
        Initialize the kernel and flatten the kernel
            ...
            x: only need 0
        """
        y_spread_ = torch.linspace(-center, center, kernel_size).to(device)
        x_spread_ = torch.zeros([kernel_size]).to(device)
...
if __name__ == "__main__":
    ...
    model = DscPatchEmb2D(in_ch, embed_dim, kernel_size, patch_size).cuda()
