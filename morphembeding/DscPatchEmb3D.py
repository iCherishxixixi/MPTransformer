...
class DscPatchEmb3D(nn.Module):
    def __init__(self, in_ch, embed_dim, kernel_size, patch_size, la_ch=1, extend_scope=3, if_offset=True):
        super(DscPatchEmb3D, self).__init__()
        product = np.prod(patch_size)
        self.conv0x = DCN_Conv(in_ch, la_ch, kernel_size, extend_scope, 0, if_offset)
        self.conv0y = DCN_Conv(in_ch, la_ch, kernel_size, extend_scope, 1, if_offset)
        self.conv0z = DCN_Conv(in_ch, la_ch, kernel_size, extend_scope, 2, if_offset)
        self.patch_embed = PatchEmbed(
                                      patch_size=patch_size,
                                      in_chans=3*la_ch+in_ch,
                                      embed_dim=embed_dim,
                                      spatial_dims=3
                                      )
    def forward(self, f):
        ...
        f_y = self.conv0y(f)
        f_z = self.conv0z(f)
        f_out = torch.cat([f_x, f_y, f_z, f], dim=1)
        embedings = self.patch_embed(f_out)
        return embedings
...
if __name__ == "__main__":
    ...
    b = DCN_Conv(3, 1, 3, extend_scope=2., morph=0, if_offset=True)   
    print(b(a).shape)
...
if __name__ == "__main__":
    ...
    model = DscPatchEmb3D(in_ch, embed_dim, kernel_size, patch_size).cuda()
