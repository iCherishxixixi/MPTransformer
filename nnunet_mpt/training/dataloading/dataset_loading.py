...
from nnunet_mpt.configuration import default_num_threads
from nnunet_mpt.paths import preprocessing_output_dir
...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
                 ...
                 pad_sides=None, sample_by_frequency=False):
        """
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            # Ints to prevent weird errors of e-32 vals on some 0s
            # global_seg_map = case_all_data[-1].astype(int)  
            # uniques_gseg, counts_gseg = np.unique(global_seg_map, return_counts=True)
            try:
                # import warnings
                # warnings.warn("median frequency sampler.. here we go...hope you know what you're doing")
                uniques_gseg, counts_gseg = properties['class_counts']
                counts_list.append((uniques_gseg, counts_gseg))
            except:
                # import warnings
                # warnings.warn("Dataset not processed with class_counts based preprocessor. Setting self.sample_by_frequency=False")
                if self.sample_by_frequency:
                    raise Exception("Don't run sample_by_frequency without preprocessing")
                # self.sample_by_frequency = False
            ...
            if not force_fg:
                ...
                if len(foreground_classes) == 0:
                    ...
                    start_fg = 0
                    if not self.sample_by_frequency:        # nnUNet default state
                        selected_class = np.random.choice(foreground_classes)
                        # print(selected_class)
                    else:
                        # Weighted choice based on class counts
                        if -1 in uniques_gseg:
                            start_fg +=1
                        if 0 in uniques_gseg:
                            start_fg +=1
                        probs = softmax(np.log10(np.median(counts_gseg[start_fg:])/counts_gseg[start_fg:])+1)
                        # print(foreground_classes, probs, uniques_gseg)
                        # probs = probs[foreground_classes] # Exclude missing classes
                        selected_class = np.random.choice(foreground_classes, p=probs)
                        # print(counts_gseg[start_fg:], probs, selected_class)
        ...
        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys, 'class_cnts': counts_list}
...
class DataLoader2D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, oversample_foreground_percent=0.0,
                 ...
                 pad_kwargs_data=None, pad_sides=None, sample_by_frequency=False):
        """
