...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    ...
    nnunet_mpt.trainig.network_training for the file that contains the trainer and instantiate it with the arguments saved in the pkl file. If checkpoint
    ...
    init = info['init']
    name = info['name']
    search_in = join(nnunet_mpt.__path__[0], "training", "network_training")
    tr = recursive_find_python_class([search_in], name, current_module="nnunet_mpt.training.network_training")
    ...
    if tr is None:
        raise RuntimeError("Could not find the model trainer specified in checkpoint in nnunet_mpt.trainig.network_training. If it "
                           "is not located there, please move it or change the code of restore_model. Your model "
                           "trainer can be located in any directory within nnunet_mpt.trainig.network_training (search is recursive)."
