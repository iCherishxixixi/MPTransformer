...
from nnunet_mpt.training.data_augmentation.custom_transforms import Convert3DTo2DTransform, Convert2DTo3DTransform, \
    MaskTransform, ConvertSegmentationToRegionsTransform
from nnunet_mpt.training.data_augmentation.default_data_augmentation import default_3D_augmentation_params
from nnunet_mpt.training.data_augmentation.downsampling import DownsampleSegForDSTransform3, DownsampleSegForDSTransform2
from nnunet_mpt.training.data_augmentation.pyramid_augmentations import MoveSegAsOneHotToData, \
...
def get_moreDA_augmentation_with_Range01(dataloader_train, dataloader_val, patch_size, params=default_3D_augmentation_params,
                            border_val_seg=-1,
                            seeds_train=None, seeds_val=None, order_seg=1, order_data=3, deep_supervision_scales=None,
                            soft_ds=False,
                            classes=None, pin_memory=True, regions=None,
    ...
    tr_transforms = []
    print("Data Augmentation includes normalization to 0,1 range")
    tr_transforms.append(RangeTransform((0,1), 'data'))
    ...
    val_transforms = []
    val_transforms.append(RangeTransform((0,1), 'data'))
    val_transforms.append(RemoveLabelTransform(-1, 0))
    if params.get("selected_data_channels") is not None:
        val_transforms.append(DataChannelSelectionTransform(params.get("selected_data_channels")))
    if params.get("selected_seg_channels") is not None:
