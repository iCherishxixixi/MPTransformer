...
import sys
sys.path.append("../network_architecture/custom_modules/custom_networks/MPT/")
from morphmap2 import DfMap, DfWin
from seclustering2 import ClusterAttn
...
class SwinTransformerBlock_kv(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if not self.shift_size > 0:
            self.dfwin1 = DfWin(self.dim, 3, 7)
            self.dfwin2 = DfWin(self.dim, 3, 7)
            
        self.cak = ClusterAttn(dim=dim, patch_size=self.window_size)
        self.cak.init_weights()
        
        self.cau = ClusterAttn(dim=dim, patch_size=self.window_size)
        self.cau.init_weights()
        
        self.cp = nn.Linear(3*dim, dim, bias=True)
    ...
    def forward(self, x, mask_matrix,skip=None,x_up=None):
        ...
        if self.shift_size > 0:
            ...
            skip = self.dfwin1(skip)
            x_up = self.dfwin2(x_up)
            attn_mask = None
        ...
        shifted_x = self.cp(torch.cat((shifted_x, ca_k, ca_u), 4))
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if not self.shift_size > 0:
            self.dfwin = DfWin(self.dim, 3, 7)
            
        self.ca = ClusterAttn(dim=dim, patch_size=self.window_size)
        self.ca.init_weights()
        self.cp = nn.Linear(2*dim, dim, bias=True)
    ...
    def forward(self, x, mask_matrix):
        ...
        if self.shift_size > 0:
            ...
            shifted_x = self.dfwin(x)
            attn_mask = None
             
        # partition windows
        new_shifted_x = shifted_x.clone()
        ...
        new_shifted_x = self.ca(new_shifted_x)
        
        shifted_x = self.cp(torch.cat((shifted_x, new_shifted_x), dim=4))
...
class BasicLayer_up(nn.Module):
    ...
    def forward(self, x,skip, S, H, W):
        ...
        mask_windows = mask_windows.view(-1,
                                         self.window_size * self.window_size * self.window_size)  # 3d??3??winds??????????????????winds???????
        attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)
        attn_mask = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(attn_mask == 0, float(0.0))
...
class Encoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_embed = PatchEmbed(
            patch_size=patch_size, in_chans=7*in_chans, embed_dim=embed_dim,
        ...
        for i_layer in range(self.num_layers-self.offset):
            layer = BasicLayer(
        ...
        self.dfmap = DfMap(1, 7, kernel_size=3)
    ...
    def forward(self, x):
        ...
        dflst = self.dfmap(x)
        dflsf = torch.stack(dflst, dim=1).squeeze(2)
        x = self.patch_embed(dflsf)
        down=[]
        ...
        for i in range(self.num_layers-self.offset):
            layer = self.layers[i]
            ...
            if self.offset != 0 and i==self.num_layers-self.offset-1:
                x = x.view(-1, Ws, Wh, Ww, self.num_features[i+1]).permute(0, 4, 1, 2, 3).contiguous()
                down.append(x)
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        # build layers
        self.layers = nn.ModuleList()
        for i_layer in range(self.num_layers-self.offset)[::-1]:
            
            layer = BasicLayer_up(
                dim=int(embed_dim * 2 ** (len(depths)-i_layer-1-self.offset)),
                input_resolution=(
                    pretrain_img_size[0] // patch_size[0] // 2 ** (len(depths)-i_layer-1-self.offset), pretrain_img_size[1] // patch_size[1] // 2 ** (len(depths)-i_layer-1-self.offset),
                    pretrain_img_size[2] // patch_size[2] // 2 ** (len(depths)-i_layer-1-self.offset)),
                depth=depths[i_layer],
                num_heads=num_heads[i_layer],
                window_size=window_size[i_layer],
                mlp_ratio=mlp_ratio,
        ...
        self.num_features = [int(embed_dim * 2 ** i) for i in range(self.num_layers-self.offset)]
    ...
    def forward(self,x,skips):
        ...
        for i in range(self.num_layers-self.offset)[::-1]:
...
class MPTnnF(nn.Module):
    ...
    def __init__(self, crop_size=[64,128,128],
        ...
        window_size=window_size
        self.model_down=Encoder(pretrain_img_size=crop_size,window_size=window_size,embed_dim=embed_dim,patch_size=patch_size,depths=depths,num_heads=num_heads,in_chans=input_channels, dummy=dummy)
        self.decoder=Decoder(pretrain_img_size=crop_size,embed_dim=embed_dim,window_size=window_size[::-1][1:],patch_size=patch_size,num_heads=num_heads[::-1][:-1],depths=depths[::-1][1:], dummy=dummy)
