...
class Conv(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size=3, stride=1):
        ...
        self.conv = nn.Conv3d(in_ch, out_ch, kernel_size, stride, padding=1)
        self.gn = nn.GroupNorm(out_ch, out_ch)
...
class Snake_Embed(nn.Module):
    def __init__(self, patch_size, in_ch, out_ch, kernel_size, extend_scope, if_offset):
        super(Snake_Embed, self).__init__()
        product = np.prod(patch_size)
        self.product = product
        self.patch_size = patch_size
        self.conv0x = DCN_Conv(in_ch, out_ch//(product*4), kernel_size, extend_scope, 0, if_offset)
        self.conv0y = DCN_Conv(in_ch, out_ch//(product*4), kernel_size, extend_scope, 1, if_offset)
        self.conv0z = DCN_Conv(in_ch, out_ch//(product*4), kernel_size, extend_scope, 2, if_offset)
        self.encode = Conv(in_ch, (out_ch//product-3*out_ch//(product*4)))
        #self.encode2 = Conv(in_ch, out_ch)
    ...
    def forward(self, f):
        ...
        f_encode = self.encode(f)
        
        f_out = torch.cat([f_x, f_y, f_z, f_encode], dim=1) 
        f_out = f_out.reshape(-1,self.product*ch,x//self.patch_size[0],y//self.patch_size[1],z//self.patch_size[2])
        #f_out = self.encode2(f)
        return f_out
...
class DCN_Conv(nn.Module):
    ...
    def forward(self, f):
        ...
        dcn = DCN(input_shape, self.kernel_size, self.extend_scope, self.morph)
...
if __name__ == "__main__":
    a = torch.randn(1,3,128,128,128)    
    b = DCN_Conv(3, 15, 3, extend_scope=2., morph=0, if_offset=True)   
    print(b(a).shape)